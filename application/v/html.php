...
<body class="dashboard">
	...
	<div class="ui vertical menu">
		...
		<a href="<?= base_url(); ?>attendance/my_attendance/" class="<? if ($type == 'My Attendance'): ?>active<? endif; ?> item"><i class="users icon"></i> My Attendances</a>
		...
		<? if ($account->type == 'HR'): ?>
			<? if ((isset($acl['attendance']) && $acl['attendance']->list > 0) || isset($acl['user']) && $acl['user']->list > 0): ?>
				<div class="ui dropdown item html-dropdown-left-menu">
					Employee Database <i class="dropdown icon"></i>
					<div class="menu">
						<? if (isset($acl['attendance']) && $acl['attendance']->list > 0): ?>
							<a href="<?= base_url(); ?>user/view/1/" class="<? if ($type == 'User'): ?>active<? endif; ?> item"><i class="users icon"></i> Employee List</a>
						<? endif; ?>

						<? if (isset($acl['attendance']) && $acl['attendance']->list > 0): ?>
							<a href="<?= base_url(); ?>attendance/view/1/" class="<? if ($type == 'Attendance'): ?>active<? endif; ?> item"><i class="id card icon"></i> Employee Attendance</a>
						<? endif; ?>
					</div>
				</div>
			<? endif; ?>
