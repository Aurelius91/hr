	...
	<script type="text/javascript">
		...
		function attendanceClick() {
			...
			$('.item-filter-button').click(function() {
				...
				$('#attendance-filter-user').val("<?= $user_id; ?>");
				$('#attendance-filter-user-container').dropdown('set selected', "<?= $user_id; ?>");
		...
		function filter(page) {
			...
			window.location.href = '<?= base_url(); ?>attendance/view/'+ page +'/'+ month +'/'+ year +'/'+ userId +'/';
		}
				...
				<div class="form-content">
					<div class="ui form">
						...
						<div class="field">
							...
							<div id="attendance-filter-user-container" class="ui search selection dropdown form-input">
								<input id="attendance-filter-user" type="hidden" class="data-important">
								<i class="dropdown icon"></i>
	...
	<div class="ui modal attendance-upload-modal">
		<i class="close icon"></i>
		<div class="header">Upload Attendance Log</div>
		<div class="form-content content">
			<div class="form-add">
				<div class="form-content">
					<div class="ui form">
						<div class="two fields">
							<div class="field">
								<label>Month</label>
								<div id="attendance-month-container" class="ui search selection dropdown form-input">
									<input id="attendance-month" type="hidden" class="data-important">
									<i class="dropdown icon"></i>
									<div class="default text">-- Select Month --</div>
									<div class="menu">
										<div class="item" data-value="01">January</div>
							...
							<div class="field">
								<label>Year</label>
								<div id="attendance-year-container" class="ui search selection dropdown form-input">
									<input id="attendance-year" type="hidden" class="data-important">
									<i class="dropdown icon"></i>
									<div class="default text">-- Select year --</div>
									<div class="menu">
										<? for ($i = $start_year; $i <= date('Y', time()); $i++): ?>
						...
						<div class="field">
							<label>Select Attendance Log File (.dat) <span class="color-red warning"></label>
							<input id="attendance-file" type="file" accept=".dat" class="data-important" placeholder="Name..">
						</div>
					</div>
				</div>
			</div>
			<div class="actions text-right">
				<div class="ui deny button form-button">Exit</div>
				<div class="ui button form-button" onclick="uploadFile();">Submit</div>
			</div>
		</div>
	</div>
	...
	<div class="main-content">
		<div class="ui top attached menu table-menu">
			...
			<div class="right menu">
				<a class="item item-upload-button">
					<i class="upload icon"></i> Upload LOG
				</a>

				<a href="<?= base_url(); ?>report/export_attendance/" class="item">
					<i class="file excel icon"></i> Export
				</a>

				<? if (isset($acl['attendance']) && $acl['attendance']->add != ''): ?>
					<a class="item item-add-button" href="<?= base_url(); ?>attendance/add/">
						<i class="add circle icon"></i> Add Attendance
					</a>
				<? endif; ?>
			</div>
		</div>
