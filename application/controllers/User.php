...
class User extends CI_Controller
{
	...
	public function account()
	{
		...
		$arr_data['arr_department'] = $this->_generate_department();
	...
	public function edit($user_id = 0)
	{
		...
		$user->address = $this->cms_function->trim_text($user->address);
		$user->birthday_display = ($user->birthday > 0) ? date('Y-m-d', $user->birthday) : '';
		$user->date_in_display = ($user->date_in > 0) ? date('Y-m-d', $user->date_in) : '';
	...
	public function view($page = 1, $filter = 'all', $query = '')
	{
		...
		$this->db->order_by("active DESC, name");
		$arr_user = $this->core_model->get('user');

		foreach ($arr_user as $user)
		{
			$user->status = ($user->active > 0) ? 'Active' : 'Resign';
		}
	...
	public function ajax_add()
	{
		...
		try
		{
			...
			foreach ($_POST as $k => $v)
			{
				...
				{
					$user_record[$k] = ($k == 'date' || $k == 'birthday' || $k == 'date_in') ? strtotime($v) : $v;
				}
			}

			if ($user_record['department_id'] > 0)
			{
	...
	public function ajax_edit($user_id)
	{
		...
		try
		{
			...
			$this->cms_function->update_foreign_field(array('attendance', 'user_access'), $user_record, 'user');
	...
	private function _generate_department()
	{
		$this->db->order_by('name');
		return $this->core_model->get('department');
	}
